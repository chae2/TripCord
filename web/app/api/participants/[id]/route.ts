...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const body = (await request.json().catch(() => null)) as { bio?: string } | null;
  if (typeof body?.bio !== "string") {
    return NextResponse.json({ ok: false, error: "bio 값이 필요해요." }, { status: 400 });
  }

  await prisma.participant.update({ where: { id: params.id }, data: { bio: body.bio } });
  revalidatePath("/");
